...
- **Типы и ошибки.** I18n обеспечивает человекопонятные сообщения (`auth.user_not_found`, `auth.invalid_password`). В случае логина все ошибки мапятся на `NotFoundException`, что может маскировать различие между неверным email/паролем. Положительный ответ `login` содержит только пользователя (`accessToken` не заполняется).
