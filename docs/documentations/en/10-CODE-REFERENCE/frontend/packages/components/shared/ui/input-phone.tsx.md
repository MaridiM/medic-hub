# File: packages\components\shared\ui\input-phone.tsx

## Location
`G:/Projects/doctor_lab/medic_hub_gemini/apps/web/src/packages/components/shared/ui/input-phone.tsx`
...
import { ComponentProps, ComponentRef, ForwardRefExoticComponent, forwardRef, useRef, useState } from 'react'
        ...
        <Popover open={isOpen} onOpenChange={setIsOpen} modal>
            <PopoverTrigger asChild>
                <Button
                    type='button'
                    variant='outline'
                    className='border-border/20 flex gap-1 rounded-s-lg rounded-e-none border border-r-0 px-3 focus:z-10'
                    disabled={disabled}
                >
...
const CountrySelectOption = ({
    ...
    return (
        <CommandItem className='gap-2' onSelect={handleSelect}>
            ...
            <span className='text-p-sm flex-1'>{countryName}</span>
            <span className='text-p-sm text-foreground/50'>{`+${RPNInput.getCountryCallingCode(country)}`}</span>
...
*Auto-generated documentation - Last updated: 2025-12-11T12:41:20.286Z*
